...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleBondNotFoundException(BondNotFoundException e) {
        log.warn("Bond not found: {}", e.getMessage());

        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(createErrorResponse(e.getErrorCode(), e.getMessage()));
    }

    @ExceptionHandler(BondIssuanceException.class)
    public ResponseEntity<Map<String, Object>> handleBondIssuanceException(BondIssuanceException e) {
        log.error("Bond issuance error: {}", e.getMessage(), e);
